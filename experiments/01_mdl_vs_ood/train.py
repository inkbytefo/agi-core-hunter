...
class ExperimentRunner:
    ...
    def setup_logging(self):
        ...
        # Log device information with robust GPU detection
        devices = jax.devices()
    ...
    def log_gpu_memory(self, step: int):
        ...
        try:
            ...
            gpu_devices = [d for d in jax.devices() if is_gpu_device(d)]
            if gpu_devices:
                ...
                try:
                    ...
                    wandb.log({
                        f"gpu_memory/device_platform": device.platform,
...
def main():
    ...
    # JAX device information with robust detection
    devices = jax.devices()
    ...
    # Debug: Show what was detected earlier vs now
    print(f"🔍 Module-level detection found: {len(_gpu_devices)} GPU(s)")
    print(f"🔍 Main function detection found: {len(gpu_devices)} GPU(s)")
